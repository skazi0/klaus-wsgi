# skz: based on wsgi_autoreload.py from klaus repo, replaced env variables with hardcoded
# options to make things simpler and adjust to deployment with nginx frontend
    ...
    def app(environ, start_response):
        if _.should_reload:
            ...
            _.inner_app = make_app(
                [os.path.join(repos_root, x) for x in os.listdir(repos_root) if not x.startswith('.')],
                *args, **kwargs
            )
...
application = make_autoreloading_app('/srv/git-repos', 'SkZ Git Repos', use_smarthttp=True, unauthenticated_push=True)
